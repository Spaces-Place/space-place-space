...
# 공간 등록
async def create_space(space: SpaceCreate, db: AsyncIOMotorClient, s3: Dict):
    # 1차적인 공간 저장
    space_dict = space.model_dump(exclude={"images"})
    space_id = await db.spaces.insert_one(space_dict).inserted_id
...
# 공간 수정
async def update_spaces(user_id: str, space: SpaceUpdate, db = Depends(get_database), s3 = Depends(s3_bucket)):
    existing_space = await db.spaces.find_one({"_id": ObjectId(space.space_id)})
    ...
    # 기존 이미지 삭제
    existing_image_paths = [f"{user_id}/{space.space_id}/{img['filename']}" for img in existing_space['images']]
    s3_client = s3["s3_client"]
    bucket_name = s3["bucket"]
    ...
    try:
        ...
        for idx, image in enumerate(space.images):
            file_extension = os.path.splitext(image.filename)[1]
            path = f"{user_id}/{space.space_id}/{idx}{file_extension}" # 예: user_id/space_id/0.png
