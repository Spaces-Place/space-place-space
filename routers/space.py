...
@space_router.get(
    "/nearby",
    response_model=List[SpaceListResponse],
    status_code=status.HTTP_200_OK,
    summary="위치 기반 공간 목록 조회",
)
...
async def create_space(
    ...
    if token_info["user_id"] != space_data.user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="로그인을 다시 해주세요"
        )

    if len(space_data.images) <= 0:
        HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"이미지를 등록해주세요"
        )

    space_id = await space_service.create_space(space_data)

    return SpaceCreateResponse(message="공간이 등록되었습니다.", space_id=str(space_id))
...
async def get_space(
    space_id: str, space_service: SpaceService = Depends(get_space_service)
):
    space = await space_service.get_space(space_id)

    return SpaceResponse(message="공간이 조회되었습니다.", **space)
...
async def update_spaces(
    ...
    if len(space_update_data.images) <= 0:
        HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="이미지를 등록해주세요"
        )

    await space_service.update_spaces(
        token_info["user_id"], space_id, space_update_data
    )
...
@space_router.post(
    "/pre-order", response_model=Dict, status_code=status.HTTP_200_OK, summary=""
)
async def pre_order_data(
    ...
    if result_data["usage_unit"] == "TIME":
        total_hours = calculate_time_difference(
            payment_request.start_time, payment_request.end_time
        )
